...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for x in ${train_set}_sp $train_dev $recog_set; do
        steps/make_fbank_pitch.sh --cmd "$train_cmd" --nj 6 --write_utt2num_frames true \
        data/${x} exp/make_fbank/${x} ${fbankdir}
    done

    # compute global CMVN
    compute-cmvn-stats scp:data/${train_set}_sp/feats.scp data/${train_set}_sp/cmvn.ark

    # dump features
    dump.sh --cmd "$train_cmd" --nj 6 --do_delta ${do_delta} \
    data/${train_set}_sp/feats.scp data/${train_set}_sp/cmvn.ark exp/dump_feats/train ${feat_tr_dir}
    dump.sh --cmd "$train_cmd" --nj 6 --do_delta ${do_delta} \
    data/${train_dev}/feats.scp data/${train_set}_sp/cmvn.ark exp/dump_feats/dev ${feat_dt_dir}
    for rtask in ${recog_set}; do
        feat_recog_dir=${dumpdir}/${rtask}/delta${do_delta}; mkdir -p ${feat_recog_dir}
        dump.sh --cmd "$train_cmd" --nj 6 --do_delta ${do_delta} \
        data/${rtask}/feats.scp data/${train_set}_sp/cmvn.ark exp/dump_feats/recog/${rtask} \
        ${feat_recog_dir}
    done
fi

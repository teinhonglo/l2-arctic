...
# 1c is as 1b,  while it introduces 'apply-cmvn-online' that does
...
if [ $stage -le 15 ]; then
  ...
  utils/mkgraph.sh \
    --self-loop-scale 1.0 data/lang${lm_affix}_test \
    $tree_dir $tree_dir/graph${lm_affix} || exit 1;
fi
...
if [ $stage -le 16 ]; then
  ...
  for data in $test_sets; do
      ...
      steps/nnet3/decode.sh \
          ...
          $tree_dir/graph${lm_affix} data/${data}_hires ${dir}/decode${lm_affix}_${data} || exit 1
  done
  wait
